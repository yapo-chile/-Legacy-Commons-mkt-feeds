...
class extractFeed(object):
    ...
    def filterCategory(self, category=None):
        ...
        if ( category == 2020 or category == 2060 ):
            filter_additional_category = " and user_id not in (3068060,4980658,688562,3263523,4317756,3762781) "
        # Filters for price
        ...
        if ( category == 3020 or category == 3040 or category == 3060 or category == 3080 ):
            filter_price = " and a.price is not null and a.price > 1000 "

        if ( category == 4020 or category == 4080 or category == 5020 or category == 5040 or category == 5060 or category == 5160 or category == 6060 or category == 6080 or category == 6100 or category == 6120 or category == 6160 or category == 6180 ):
    ...
    def generateCaseUrl(self):
        ...
        for c_key, c_value in categories.items():
            for r_key, r_value in region.items():
                if c_key == 3060:
                    out += """
                        when (a.region = %d and a.category = %d) then 'https://www.yapo.cl/chile?q='||replace(replace(replace(replace(replace(replace(lower(a.subject), 'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'), ' ', '+')
                        """ % (r_key, c_key )
                elif c_key == 1220 or c_key == 1240:
                    out += """
                        when (a.region = %d and a.category = %d) then 'https://www.yapo.cl/%s/%s/'||replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(lower(a.subject),' ','_'),'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'),'!','_'),'?','_'),',','_'),';','_'),':','_'),'¨','_'),'º','_'),'ª','_'),'$','_'),'#','_'),'&','_')||'_'||a.list_id||'.htm'
                    """ % (r_key, c_key, r_value, c_value)
                else:
                    out += """
                        when (a.region = %d and a.category = %d) then 'https://www.yapo.cl/%s/%s/'||replace(lower(a.subject),' ','_')||'_'||a.list_id||'.htm'
                    """ % (r_key, c_key, r_value, c_value)
        out += "end as url,"
        return out
    ...
    def extractProductFeed(self, category=None, ad_ids_filter=""):
        ...
        category       numeric  id of subcategory that filter query.  1220
        ad_ids_filter  string   Set of ad_id split by ","             349087,456645,7665
        -------
        Return : Dictionary
        """
        ...
        from (select
            ...
            replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(a.subject,'á','a'),'é','e'),'í','i'),'ó','o'),'ú','u'),'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U') as name,
            case
                when length(am.ad_media_id::text) < 10 then 'https://img.yapo.cl/images/0'||substring(am.ad_media_id::text, 1, 1)||'/0'||(am.ad_media_id::text)||'.jpg'
                else 'https://img.yapo.cl/images/'||substring(am.ad_media_id::text, 1, 2)||'/'||(am.ad_media_id::text)||'.jpg'
            end as image_url,
            case
                when a.category-mod(a.category,1000) = 1000 then 'Inmuebles'
                when a.category-mod(a.category,1000) = 2000 then 'Vehiculos'
                when a.category-mod(a.category,1000) = 3000 then 'Computadores & electronica'
                when a.category-mod(a.category,1000) = 4000 then 'Moda, calzado, belleza y salud'
                when a.category-mod(a.category,1000) = 5000 then 'Hogar'
                when a.category-mod(a.category,1000) = 6000 then 'Tiempo Libre'
                when a.category-mod(a.category,1000) = 9000 then 'Futura mamá, bebés y niños'
            end as main_category,
            case
                when a.category = 1220 then 'Comprar'
                when a.category = 1240 then 'Arrendar'
                when a.category = 2020 then 'Autos, camionetas y 4x4'
                when a.category = 2060 then 'Motos'
                when a.category = 3020 then 'Consolas, videojuegos y accesorios'
                when a.category = 3040 then 'Computadores y accesorios'
                when a.category = 3060 then 'Celulares, teléfonos y accesorios'
                when a.category = 3080 then 'Audio, TV, video y fotografia'
                when a.category = 4020 then 'Moda y vestuario'
                when a.category = 4040 then 'Bolsos, bisutería y accesorios'
                when a.category = 4060 then 'Salud y belleza'
                when a.category = 4080 then 'Calzado'
                when a.category = 5020 then 'Muebles'
                when a.category = 5040 then 'Electrodomésticos'
                when a.category = 5060 then 'Jardín y herramientas'
                when a.category = 5160 then 'Otros artículos del hogar'
                when a.category = 6020 then 'Deportes, gimnasia y accesorios'
                when a.category = 6060 then 'Bicicletas, ciclismo y accesorios'
                when a.category = 6080 then 'Instrumentos musicales y accesorios'
                when a.category = 6100 then 'Música y películas (DVDs, etc.)'
                when a.category = 6120 then 'Libros y revistas'
                when a.category = 6140 then 'Animales y sus accesorios'
                when a.category = 6160 then 'Arte, antigüedades y colecciones'
                when a.category = 6180 then 'Hobbies y outdoor'
                when a.category = 9020 then 'Vestuario futura mamá y niños'
                when a.category = 9040 then 'Juguetes'
                when a.category = 9060 then 'Coches y artículos infantiles'
            end as category,
            replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(a.body,'á','a'),'é','e'),'í','i'),'ó','o'),'ú','u'),'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U') as description,
            a.price::text,
            case
            ...
            left join
                ...
                union all
                select * from blocket_2018.mail_queue) mq on a.list_id = mq.list_id
            where
