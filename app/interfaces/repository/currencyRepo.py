...
class CurrencyRepo():
    ...
    # getRawCatalogWithFixedPrice returns a dataframe with price on chilean pesos
    ...
    # getCurrentUF returns updated uf from external api. If it fails, gets latest or default value
    ...
    # _getUpdatedUF returns UF value from a external api. IF something fails returns 0
    ...
    # _getUFPrice if currency is UF, it converts this value to chilean pesos. otherwise returns price
    def _getUFPrice(self, price, currency, uf) -> int:
        if isinstance(currency, str) and currency == 'uf':
            print("price {} - normalize {} - uf {}".format(price, self.ufConf.normalizeFactor, uf))
