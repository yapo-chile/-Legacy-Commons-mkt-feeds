...
class extractFeed(object):
    ...
    def extractProductFeed(self, category=None, ad_ids_filter=""):
        ...
        filter_additional_category,
        filter_price,
        group_by = self.filterCategory(category)
        ...
        self.log.getLogInfo("Extract feed from category "+str(category))
        ...
        from (select
            ...
            replace(replace(replace(replace(replace(replace(replace(replace(
                replace(replace(a.body,'á','a'),'é','e'),'í','i'),'ó','o')
...
def getFeedToEndpoint(category=None, position=1):
    ...
    sequence = ["ad_id",
                ...
                "num_ad_replies"]
    ef = extractFeed()
    filter = '80778127,80737379,80112070,81736906'
    data = pd.DataFrame(ef.extractProductFeed(category), ad_ids_filter=filter)
    print(data.head())
    # data.to_csv(fileName,
    # index=False, columns=sequence, encoding='utf-8', sep=';')
...
if __name__ == '__main__':
    ...
    categoryList = [1220]
    getFeedToEndpoint(category=1220, position=1)
    countCategory = 1
